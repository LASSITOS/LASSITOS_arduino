...
uint8_t txValue = 0;
char txString[1000];   // String containing messages to be send to BLE terminal
char subString[20];
...
void stop_logging() {
	...
	OpenLog.write(13); //This is a new line
...
void setDynamicModel( String rxValue){  
   if (!logging) {
	  ...
	  if (index !=-1 and index2 !=-1){
  		...
  		DynamicModel = rxValue.substring(index+1,index2).toInt();
  		BLE_message=true;
...
//    strcat(txString,"Prova");
    ...
    {
      strcat(txString,"X Ang Rate: ");
      sprintf(subString,"%d",txString,myGNSS.packetUBXESFINS->data.xAngRate);
      strcat(txString,subString);  
      strcat(txString," Y Ang Rate: ");
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.yAngRate);
      strcat(txString,subString);  
      strcat(txString," Z Ang Rate: ");
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.zAngRate);
      strcat(txString,subString);  
      strcat(txString," X Accel: ");
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.xAccel);  
      strcat(txString,subString);
      strcat(txString," Y Accel: ");
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.yAccel);
      strcat(txString,subString);  
      strcat(txString," Z Accel: ");
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.zAccel);
      strcat(txString,subString);
        
      // These values also have "validity checks" that can be provided by the
      // ublox library by reading bitfield0
      strcat(txString," Validity: ");
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.bitfield0.bits.xAngRateValid);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.bitfield0.bits.yAngRateValid);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.bitfield0.bits.zAngRateValid);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.bitfield0.bits.xAccelValid);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.bitfield0.bits.yAccelValid);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFINS->data.bitfield0.bits.zAccelValid);
      strcat(txString,"\n");
    }
    // ESF data is produced at the navigation rate, so by default we'll get fresh data once per second
    if (myGNSS.getEsfAlignment()) // Poll new ESF ALG data
    {
      strcat(txString,"Status: "); 
      sprintf(subString,"%d",myGNSS.packetUBXESFALG->data.flags.bits.status);
      strcat(txString," Roll: "); 
      sprintf(subString,"%.2d",myGNSS.getESFroll()); // Use the helper function to get the roll in degrees
      strcat(txString,subString);
      strcat(txString," Pitch: "); 
      sprintf(subString,"%.2d",myGNSS.getESFpitch()); // Use the helper function to get the pitch in degrees
      strcat(txString,subString);
      strcat(txString," Heading: "); 
      sprintf(subString,"%.2d",myGNSS.getESFyaw()); // Use the helper function to get the yaw in degrees
      strcat(txString,subString);
      strcat(txString," Errors: "); 
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFALG->data.error.bits.tiltAlgError);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFALG->data.error.bits.yawAlgError);
      strcat(txString,subString);
      sprintf(subString,"%d",myGNSS.packetUBXESFALG->data.error.bits.angleError);
      strcat(txString,subString);
      strcat(txString,"\n");
    }
